...
{
    if (!context || !out_value || port_index >= 8) return RTSYN_ABI_STATUS_INVALID_ARGUMENT;
...
{
    if (!context || !value || port_index >= 8) return RTSYN_ABI_STATUS_INVALID_ARGUMENT;
...
TEST(RTHYBRID_AMPLITUDE_SCALE_OFFSET, DescriptorShape)
{
    const auto *descriptor = rtsyn_rthybrid_amplitude_scale_offset_get_descriptor();
    ASSERT_NE(descriptor, nullptr);
    EXPECT_STREQ(descriptor->name, "rthybrid_amplitude_scale_offset");
    EXPECT_EQ(descriptor->node_type, RTSYN_ABI_NODE_PLUGIN);
    EXPECT_EQ(descriptor->port_count, 8U);
    EXPECT_EQ(descriptor->param_count, 0U);
    EXPECT_EQ(descriptor->state_count, 8U);
}

TEST(RTHYBRID_AMPLITUDE_SCALE_OFFSET, ProcessesAndPublishesFiniteOutput)
{
    const auto *descriptor = rtsyn_rthybrid_amplitude_scale_offset_get_descriptor();
...
TEST(RTHYBRID_AMPLITUDE_SCALE_OFFSET, StateAndInvalidAccess)
{
    const auto *descriptor = rtsyn_rthybrid_amplitude_scale_offset_get_descriptor();
    void *instance = nullptr;
    double value = 1.0;
